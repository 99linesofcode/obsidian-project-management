...
// UC4: push a note's content onto its GitHub issue. The body is projected for
// GitHub first — checklist wikilinks are vault-only and never reach the issue.
// The title is derived from the note's filename slug (dashes → spaces). The
// slug round-trips lossily — it cannot recover the original casing or
// punctuation — but readably; the filename slug is the user's chosen naming,
// so it wins.
